import { cardColorsFor } from './cardColors'
import { MiniModifierCard } from './MiniModifierCard'

const MINI_W = 18
const MINI_H = 26

const GLOW = '0 0 0 2px rgba(14, 165, 233, 0.45), 0 2px 4px rgba(14, 165, 233, 0.35)'

export const MINI_DIMENSIONS = { width: MINI_W, height: MINI_H, modifierWidth: 22 }

export function MiniCard({ card, glow = false, style }) {
  if (!card) return null
  if (card.type === 'number') return <Num value={card.value} glow={glow} style={style} />
  if (card.type === 'modifier') return <MiniModifierCard card={card} glow={glow} style={style} />
  if (card.type === 'action') {
    if (card.action === 'secondChance') {
      return <MiniModifierCard card={card} glow={glow} style={style} />
    }
    return <Act card={card} glow={glow} style={style} />
  }
  return null
}

function Num({ value, glow, style }) {
  const c = cardColorsFor(value)
  return (
    <div
      style={{
        width: MINI_W,
        height: MINI_H,
        borderRadius: 4,
        border: `1.5px solid ${c.border}`,
        background: `linear-gradient(135deg, ${c.wash1}, ${c.wash2})`,
        boxShadow: glow ? GLOW : null,
        display: 'flex',
        alignItems: 'center',
        justifyContent: 'center',
        ...style,
      }}
    >
      <span
        style={{
          background: 'rgba(255,255,255,0.88)',
          borderRadius: 4,
          padding: '1px 3px',
          fontFamily: "'Nunito', sans-serif",
          fontSize: 9,
          fontWeight: 900,
          color: c.text,
          lineHeight: 1,
        }}
      >
        {value}
      </span>
    </div>
  )
}

function Act({ card, glow, style }) {
  return (
    <div
      style={{
        width: 22,
        height: MINI_H,
        borderRadius: 4,
        border: '1.5px solid #818CF8',
        background: '#E0E7FF',
        boxShadow: glow ? GLOW : null,
        color: '#312E81',
        fontFamily: "'Nunito', sans-serif",
        fontWeight: 900,
        fontSize: 8,
        display: 'flex',
        alignItems: 'center',
        justifyContent: 'center',
        ...style,
      }}
    >
      {card.action === 'freeze' ? 'frz' : '×3'}
    </div>
  )
}
